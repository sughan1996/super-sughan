import json

import requests

from haikuFoundationApp.processRequestHandler import lambda_handler

url = "https://cognito-idp.us-east-1.amazonaws.com/"
payload = "{\n  \"AuthFlow\": \"USER_PASSWORD_AUTH\",\n  \"ClientId\": \"3q34c48o6fvaqbdmssvuovu86k\",\n  \"AuthParameters\": {\n    \"USERNAME\": \"samsonbabuji\",\n    \"PASSWORD\": \"Samsonbabuji@1967!\"\n  }\n}"
headers = {
    'Content-Type': 'application/x-amz-json-1.1',
    'X-Amz-Target': 'AWSCognitoIdentityProviderService.InitiateAuth'
}

response = requests.request("POST", url, headers=headers, data=payload).json()

access_token = response['AuthenticationResult']['AccessToken']
event_0 = {
    "httpMethod": "POST",
    "body": json.dumps({"requestMethod": "/poems", "userId": "samsonbabuji"}),
    "headers": {
        "Authorization": 'Bearer ' + access_token}}
event_1 = {
    "httpMethod": "POST",
    "body": json.dumps({"requestMethod": "/topics", "userId": "samsonbabuji"}),
    "headers": {
        "Authorization": 'Bearer ' + access_token}}

event_2 = {
    "httpMethod": "POST",
    "body": json.dumps({"requestMethod": "/explore", "userId": "samsonbabuji"}),
    "headers": {
        "Authorization": 'Bearer ' + access_token}}


event_3 = {
    "httpMethod": "POST",
    "body": json.dumps({"requestMethod": "/international", "userId": "samsonbabuji"}),
    "headers": {
        "Authorization": 'Bearer ' + access_token}}

event_4 = {
    "httpMethod": "POST",
    "body": json.dumps({"requestMethod": "/society", "userId": "samsonbabuji"}),
    "headers": {
        "Authorization": 'Bearer ' + access_token}}


event_5 = {
    "httpMethod": "POST",
    "body": json.dumps({"requestMethod": "/lifestyle", "userId": "samsonbabuji"}),
    "headers": {
        "Authorization": 'Bearer ' + access_token}}

event_5 = {
    "httpMethod": "POST",
    "body": json.dumps({"requestMethod": "/sports", "userId": "samsonbabuji"}),
    "headers": {
        "Authorization": 'Bearer ' + access_token}}


event_6 = {
    "httpMethod": "POST",
    "body": json.dumps({"requestMethod": "/saved", "userId": "samsonbabuji"}),
    "headers": {
        "Authorization": 'Bearer ' + access_token}}


event_6 = {
    "httpMethod": "POST",
    "body": json.dumps({"requestMethod": "/saved", "userId": "sughanrichardson"}),
    "headers": {
        "Authorization": 'Bearer ' + access_token}}

if __name__ == "__main__":
    print(lambda_handler(event_6, None))
    # print(lambda_handler(event_1, None))
    # print(lambda_handler(event_2, None))

